        ...
        */
        while ! host_list.is_empty() {  
            ...
            if health_check == "None" {
                ...
                return Err(anyhow!(format!("Index clear scheduling Stopped :: [{} cluster - health is {}]", cluster_name, health_check)));
            }
        ...
        if ! conn_check {
            return Err(anyhow!(format!("Index clear scheduling failed :: Connection to {} cluster failed.", cluster_name)));
        }
