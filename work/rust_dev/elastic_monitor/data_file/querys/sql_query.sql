...
alter table nosql_mon_bot add column role_type int;
...
,	MAX(CASE WHEN nlm.metric_type = 'memory' THEN nlm.limit_value ELSE 0 END) AS memory_limit
,	MAX(CASE WHEN nlm.metric_type = 'm_s_sync' THEN nlm.limit_value ELSE 0 END) AS sync_limit
,   nct.cluster_group_name
...
WHERE nct.system_type = 'REDIS'
GROUP BY nct.cluster_name;

SELECT
	CONCAT(host_ip, ':', host_port) as host_info
FROM NOSQL_HOST_INFO
WHERE cluster_name = 'REDIS_FARM_0_DEV'
AND system_type = 'REDIS';
...
alter table NOSQL_CLUSTER_TYPES add column cluster_group_name VARCHAR(100);

select * from NOSQL_CLUSTER_TYPES;

update NOSQL_CLUSTER_TYPES set cluster_group_name = cluster_name where system_type = 'ES';


select * from NOSQL_MON_BOT;


RENAME TABLE NOSQL_MON_BOTS TO NOSQL_MON_BOT;
...
,	system_version
,	ssl_option
FROM NOSQL_CLUSTER_TYPES
WHERE cluster_name = 'es_infra_monitoring';
...
DELETE FROM NOSQL_CLUSTER_TYPES where system_type = "REDIS";

DELETE FROM NOSQL_HOST_INFO where system_type = "REDIS";


SELECT * FROM NOSQL_HOST_INFO where system_type = "REDIS";


SELECT * FROM NOSQL_CLUSTER_TYPES where system_type = "REDIS";
...
delete from NOSQL_LIMIT_METRICS where metric_name = 'master_slave_sync';

insert into NOSQL_LIMIT_METRICS values ('mem_5','memory',5,now(),null);
insert into NOSQL_LIMIT_METRICS values ('master_slave_sync_11', 'm_s_sync', 11, now(), null);


select * from NOSQL_MON_GROUP;

insert into NOSQL_MON_GROUP values ('test_redis_metric_set', 'master_slave_sync_11', 1, now(), null);
...
insert into NOSQL_MON_GROUP values ('test_redis_metric_set', 'disk_20', 1, now(), null);
insert into NOSQL_MON_GROUP values ('test_redis_metric_set', 'cpu_20', 1, now(), null);
...
insert into NOSQL_INDEX_SCHEDULE values ('ES', 'es_infra_monitoring', 'seunghwan-test', 1, now(), null);
insert into NOSQL_INDEX_SCHEDULE values ('ES', 'es_infra_monitoring', 'soeyuni-test', 1, now(), null);

insert into NOSQL_INDEX_SCHEDULE values ('ES', 'ad-es-dev', 'kafka_index_test', 1, now(), null);


select * from NOSQL_MON_BOTS;

CREATE TABLE NOSQL_MON_BOTS
(
  bot_token       VARCHAR(300) NOT NULL,
  chat_room_id       VARCHAR(300) NOT NULL,
  mon_apply_yn INT          NOT NULL,
  PRIMARY KEY (bot_token, chat_room_id)
);

drop table NOSQL_MON_BOTS;

insert into NOSQL_MON_BOTS values ('6053700278:AAEXtjW4gH6EfAjygT8vTvyvwO9k8_lxxAY', '-1001588470751', 1);
insert into NOSQL_MON_BOTS values ('6053700278:AAEXtjW4gH6EfAjygT8vTvyvwO9k8_lxxAY', '-992602912', 1);
...
SELECT * FROM NOSQL_HOST_INFO
where cluster_name = 'cataloglisting-engine';

insert into NOSQL_LIMIT_METRICS values ('es_shard_20', 'shard', 20 ,now(),null);
