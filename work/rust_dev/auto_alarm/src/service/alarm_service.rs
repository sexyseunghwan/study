...
*/
pub async fn push_alarm_to_telebot(kafka_client: KafkaBroker, mysql_client: MySqlAsyncClient) -> Result<(), anyhow::Error> {
    ...
    let handle2 = spawn(async move {
        let _ = kafka_client.consume_and_send_messages("nosql_mon_log", shared_res_clone_handle2).await;
    });
